...
CREATE TABLE pet_user (
  ...
  auth_no INTEGER      NULL     COMMENT '인증번호' -- 인증번호
)
COMMENT '회원';
=======
CREATE TABLE `pet_user` (
...
>>>>>>> 1daa6919eb6a5d86fc7425c73ae82eb5b1519a43
...
CREATE TABLE pet_hp_review (
  ...
  writer INTEGER      NOT NULL COMMENT '작성자', -- 작성자
  serv   INTEGER      NOT NULL COMMENT '서비스', -- 서비스
...
CREATE TABLE pet_st_scrap (
  ...
  is_scrap INTEGER NOT NULL DEFAULT 0 COMMENT '스크랩여부' -- 스크랩여부
)
COMMENT '스크랩';
...
CREATE TABLE pet_qna (
  ...
  state  INTEGER     NOT NULL DEFAULT 0 COMMENT '답변여부' -- 답변여부
)
COMMENT 'Q&A';
...
CREATE TABLE pet_hp_record (
  ...
  hname INTEGER  NOT NULL COMMENT '병원명', -- 병원명
  hist  LONGTEXT NOT NULL COMMENT '진찰내역' -- 진찰내역
)
COMMENT '진찰기록';
